...
const SettingsModal = ({ isOpen, onClose, themes, themeName, setThemeName, layoutDensity, setLayoutDensity, fontSize, setFontSize }) => {
    ...
    // Using a simple debounce for interactive clicks if needed, but for now direct call is fine.
    const handleInteractiveClick = (callback) => (...args) => {
        // Can add haptic feedback here if desired:
                    ...
                    <div className="border-t border-outline-variant pt-4 space-y-4">
                        ...
                        <div>
                            ...
                            <div ref={layoutContainerRef} className="relative flex rounded-full p-1 bg-surface-container-high border border-outline-variant shadow-inner">
                                ...
                                <button ref={comfortableBtnRef} onClick={handleInteractiveClick(() => setLayoutDensity('comfortable'))} className={`relative w-full py-1.5 font-semibold rounded-full transition-colors ${layoutDensity === 'comfortable' ? 'text-on-primary' : 'text-on-surface-variant'}`}>Comfortable</button>
                                <button ref={compactBtnRef} onClick={handleInteractiveClick(() => setLayoutDensity('compact'))} className={`relative w-full py-1.5 font-semibold rounded-full transition-colors ${layoutDensity === 'compact' ? 'text-on-primary' : 'text-on-surface-variant'}`}>Compact</button>
                            </div>
                        </div>
                        <div>
                            ...
                            <div ref={fontContainerRef} className="relative flex rounded-full p-1 bg-surface-container-high border border-outline-variant shadow-inner">
                                ...
                                <button ref={smallBtnRef} onClick={handleInteractiveClick(() => setFontSize('sm'))} className={`relative w-full py-1.5 font-semibold rounded-full transition-colors ${fontSize === 'sm' ? 'text-on-primary' : 'text-on-surface-variant'}`}>Small</button>
                                <button ref={baseBtnRef} onClick={handleInteractiveClick(() => setFontSize('base'))} className={`relative w-full py-1.5 font-semibold rounded-full transition-colors ${fontSize === 'base' ? 'text-on-primary' : 'text-on-surface-variant'}`}>Default</button>
                                <button ref={largeBtnRef} onClick={handleInteractiveClick(() => setFontSize('lg'))} className={`relative w-full py-1.5 font-semibold rounded-full transition-colors ${fontSize === 'lg' ? 'text-on-primary' : 'text-on-surface-variant'}`}>Large</button>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="p-6 pt-4 mt-auto flex-shrink-0 border-t border-outline-variant">
                    <button onClick={onClose} className="w-full py-2.5 font-bold rounded-full bg-primary text-on-primary shadow-md hover:opacity-90 transition-opacity active:scale-95">Done</button>
                </div>
            </div>
        </div>
    );
